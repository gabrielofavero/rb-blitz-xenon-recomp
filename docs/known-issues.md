...
| **No unit tests and no automated acceptance run** | The port has zero automated tests, so fixes are "verified once and forgotten". Pure host code (starting with the B-009 key-table deobfuscation) can be tested without booting the game. The only scripted acceptance test is [scripts/acceptance_launches.ps1](../scripts/acceptance_launches.ps1), which stops at boot — there is nothing that drives menus → song → results, so milestones 4 and 5 have no recordable pass/fail. | [rb3-references.md](rb3-references.md) §7.3 |
