...
| **Most booted behaviour is still verified by hand; host test coverage is two targets** | Fixes are still "verified once and forgotten" wherever proving them needs a boot. [tests/](../tests) covers the two pieces of pure host code — `rb_blitz_crypto_keytable_tests` (the B-009 key path through [src/hooks/crypto_keytable.h](../src/hooks/crypto_keytable.h)) and `rb_blitz_fingerprint_tests` (the parser and verdicts behind [src/util/game_fingerprint.h](../src/util/game_fingerprint.h)), 419 checks in total, all under `ctest` in the configured build tree. Two scripted acceptance runs now cover the offline UI route: [scripts/acceptance_launches.ps1](../scripts/acceptance_launches.ps1) stops at the title screen, and [scripts/acceptance_song.ps1](../scripts/acceptance_song.ps1) drives launch → a named song → results (and, with `-Replay`, a second song) while keeping the screenshots and the log of every run. Everything the route does not visit still needs a boot: every hook in [src/hooks/](../src/hooks) that touches the SDK, and the UI paths outside the offline song loop (pause/resume, pad lane input, settings, save restart persistence). | [build-and-run.md](build-and-run.md) §3–§4, [rb3-references.md](rb3-references.md) §7.3 |
