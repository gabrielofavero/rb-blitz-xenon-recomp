...
| **No automated acceptance run; host test coverage is one target** | Fixes are still "verified once and forgotten" wherever proving them needs a boot. [tests/](../tests) now holds the first host unit tests — `rb_blitz_crypto_keytable_tests` covers the B-009 key path through [src/hooks/crypto_keytable.h](../src/hooks/crypto_keytable.h) and runs under `ctest` in the configured build tree — but that one header is the only pure host code with coverage, and the only scripted acceptance test is [scripts/acceptance_launches.ps1](../scripts/acceptance_launches.ps1), which stops at boot. Nothing drives menus → song → results, so milestones 4 and 5 still have no recordable pass/fail. | [build-and-run.md](build-and-run.md) §3, [rb3-references.md](rb3-references.md) §7.3 |
| **`config/game_fingerprints.toml` is consumed by nothing** | The fingerprint file is committed but no code reads it: nothing in the SDK references `game_fingerprints`, and a boot log contains no fingerprint, image hash, or SDK version line. A wrong retail dump therefore fails late and confusingly instead of failing closed, and the plan's definition-of-working #10 ("log containing the SDK version, game fingerprint, last guest PC") is only partly satisfied. | [DECOMPILATION_PLAN.md](../DECOMPILATION_PLAN.md), [docs/build-and-run.md](build-and-run.md) |
