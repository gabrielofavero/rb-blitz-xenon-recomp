...
| Fingerprinting must not fail closed on an overlay | `config/game_fingerprints.toml` describes the vanilla dump, and a Deluxe root changes `gen/` and the `.xex`. **Answered 2026-09-19:** the build gate fails closed on `default.xex` and can be relaxed with `-DRBBLITZ_ALLOW_MODIFIED_GAME_DATA=ON`, while the boot-time identity check only warns — neither claims a Deluxe image *compiles*. | [known-issues.md](known-issues.md), [build-and-run.md](build-and-run.md) §3 |
